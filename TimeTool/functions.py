...
def LSTM_diy(data,split):


    #data = pd.read_csv(path, parse_dates=True, index_col=0, encoding='gb18030')
    ...
    # 拟合模型
    history = model.fit(train_X, train_y[:, 3], epochs=50, batch_size=30, validation_data=(test_X, test_y), verbose=2,
                        shuffle=False)
    # 绘制损失趋势线
